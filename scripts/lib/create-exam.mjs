// Helper for creating new entrance_exams rows with humanizer preflight.
// Mirrors replace-scheme.mjs but writes to entrance_exams and posts to
// /entrance-exams/{slug}. Reads Supabase creds from .env.local.
...
function preflight(row, minWords) {
  ...
  const bundle = [row.title, row.description, row.content, row.eligibility || "", row.syllabus || "", row.exam_pattern || ""].join("\n");
...
export async function createExam(row, minWords = 3000) {
  const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
  console.log(`\n=== Creating /entrance-exams/${row.slug} ===`);
  ...
  console.log(`  Preflight OK: title=${row.title.length}c desc=${row.description.length}c content=${contentWords}w`);
  ...
  const { data: existing } = await supabase.from("entrance_exams").select("id").eq("slug", row.slug).maybeSingle();
  if (existing) {
    const { error } = await supabase.from("entrance_exams").update(insertRow).eq("slug", row.slug);
    if (error) { console.error("  FAIL (update):", error.message); process.exit(1); }
    console.log(`  OK entrance_exams/${row.slug} (updated existing row)`);
  } else {
    const { error } = await supabase.from("entrance_exams").insert(insertRow);
    if (error) { console.error("  FAIL (insert):", error.message); process.exit(1); }
    console.log(`  OK entrance_exams/${row.slug} (inserted new row)`);
  }
